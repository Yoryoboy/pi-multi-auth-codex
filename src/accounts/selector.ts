...
function eligible(a: Account, now: number, repair: boolean) { return a.enabled && (a.rateLimitedUntil === null || a.rateLimitedUntil <= now) && (repair || a.authInvalidAt === null || a.authInvalidAt + AUTH_INVALID_COOLDOWN_MS <= now); }
...
export async function selectAccount(store: AccountStore, options: AccountSelectionOptions = {}): Promise<CodexCredentialSnapshot> {
  ...
  await store.mutate((current: Store) => {
    ...
    const candidates = current.accounts.filter(a => !options.excludeAccountKeys?.has(a.id) && eligible(a, now, !!options.allowAuthInvalid));
    ...
    const selected = Array.from({ length: current.accounts.length }, (_, i) => current.accounts[(start + i) % current.accounts.length]).find(a => !options.excludeAccountKeys?.has(a.id) && eligible(a, now, !!options.allowAuthInvalid));
