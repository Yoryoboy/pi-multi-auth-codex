import type { Account, AccountStore, RoutingStrategy, Store } from "./store.js";
...
export function validateAlias(alias: string): string | undefined {
  if (!ALIAS_PATTERN.test(alias))
    return "Alias must be 1-32 characters: letters, numbers, dot, dash, or underscore.";
}
...
export function accountStatus(
  ...
  if (account.rateLimitedUntil !== null && account.rateLimitedUntil > now)
    return "rate-limited";
  if (account.authInvalidAt !== null && account.authInvalidAt + 300_000 > now)
...
export function maskEmail(email?: string): string {
  ...
  if (!/^[^@]+$/.test(local) || !/^[A-Za-z0-9.-]+$/.test(domain))
    return REDACTED_EMAIL;
...
export function renderAccount(
  account: Account,
  store: { lastSelectedAccountId?: string | null },
  now = Date.now(),
): string {
...
): Promise<Store | { uncertain: true }> {
  try {
    return await store.mutate((current) =>
      current.routingStrategy === strategy
        ? current
        : { ...current, routingStrategy: strategy },
    );
  } catch (error) {
    if (error instanceof StoreCommitUncertainError) return { uncertain: true };
    throw error;
  }
}
...
export async function removeAccount(
  ...
  try {
    ...
    await store.mutate((current) => {
      ...
      const predecessor =
        removedIndex > 0
          ? current.accounts[removedIndex - 1]?.id
          : current.accounts.at(-1)?.id;
      return {
        ...current,
        accounts,
        lastSelectedAccountId:
          current.lastSelectedAccountId === id
            ? accounts.length === 0
              ? null
              : (predecessor ?? null)
...
export interface ReauthenticationExpectation {
  accessToken: string;
  refreshToken: string;
  accountId: string;
}
export class ReauthenticationConflictError extends Error {
  ...
  constructor() {
    super(
      "Account changed during reauthentication; no credentials were overwritten",
    );
    Object.defineProperty(this, "name", {
      value: "ReauthenticationConflictError",
      enumerable: false,
    });
...
export async function reauthenticateAccount(
  ...
  tokens: OAuthTokens,
  now: number,
  expected: ReauthenticationExpectation,
): Promise<ManageMutationResult> {
  ...
  try {
    ...
    await store.mutate((current) => {
      ...
      if (found.accountId !== identity.accountId)
        throw new Error("OAuth identity does not match the selected account");
      if (
        found.id !== id ||
        found.accountId !== expected.accountId ||
        found.accessToken !== expected.accessToken ||
        found.refreshToken !== expected.refreshToken
      )
        throw new ReauthenticationConflictError();
      const {
        rateLimitedUntilByModel: _staleCooldowns,
        ...withoutModelCooldown
      } = found;
      account = {
        ...withoutModelCooldown,
        accessToken: tokens.access_token,
        refreshToken: tokens.refresh_token ?? found.refreshToken,
        idToken: tokens.id_token ?? found.idToken,
        email: identity.email ?? found.email,
        planType: identity.planType ?? found.planType,
